/**
 * Print-ready HTML for partner readings — mirrors PartnerReadingView + partner-reading.css
 * (no sidebar). Used for Chromium print-to-PDF so output matches browser "Save as PDF".
 */
...
function blockHtml(block: any): string {
  ...
  const insight = block?.insightText
    ? `<aside class="pr-insight">
        <div class="pr-insight-label">${esc(block.insightLabel || "AI Insight")}</div>
        <p class="pr-insight-text">${esc(block.insightText)}</p>
      </aside>`
    : "";
...
function formatDob(dob?: string | null): string {
  if (!dob || dob === "—" || dob === "Stellar Origin") return "—";
...
/** Normalize any common DOB shape → YYYY-MM-DD (or null) */
function toIsoDob(input?: string | null): string | null {
...
function calcAge(dob?: string | null): number | null {
  const iso = toIsoDob(dob) || dob;
  if (!iso) return null;
  const d = new Date(iso.includes("T") ? iso : `${toIsoDob(iso) || iso}T12:00:00`);
...
:root {
  ...
  --ink-muted: #7569AA;
  --accent: #5D4BE0;
  --accent-soft: rgba(93,75,224,0.10);
  --accent-border: rgba(93,75,224,0.30);
  --border: rgba(40,22,110,0.10);
...
.hero-blob {
  ...
  background:
    radial-gradient(circle at 35% 35%, rgba(255,255,255,0.95) 0%, rgba(255,255,255,0.35) 35%, transparent 70%),
    linear-gradient(135deg, #5D4BE0 0%, #9B82FF 100%);
  border-radius: 50% 40% 55% 45% / 50% 50% 45% 55%;
  display: grid; place-items: center;
  font-family: var(--font-display); font-size: 48px; font-weight: 500; color: white;
}
.hero-eyebrow {
...
.hero-title {
  font-size: 40px; font-weight: 500; line-height: 1.08;
...
.hero-meta-row {
  ...
  background: var(--card); font-size: 11px; letter-spacing: 0.14em;
...
/* Chapters MUST split across pages — avoid-page on tall blocks causes blank pages */
.chapter {
...
.chapter-head {
  text-align: center; padding: 32px 0 22px;
  border-bottom: 1px solid var(--border); margin-bottom: 22px; position: relative;
  /* Keep title with the first bit of body, but allow the chapter itself to split */
...
.pr-sub.level-h2 .pr-sub-title {
  font-size: 20px; padding-bottom: 8px; border-bottom: 1px solid var(--border-strong);
  margin: 0 0 14px; color: var(--accent); font-weight: 500;
}
.pr-sub.level-h3 .pr-sub-title {
  font-size: 16px; margin: 0 0 10px; color: var(--ink); font-weight: 500;
}
.pr-sub p { font-size: 14.5px; line-height: 1.75; margin: 0 0 10px; }
.pr-sub ul { padding: 4px 0; margin: 8px 0; }
.pr-sub ul li {
  font-size: 14px; line-height: 1.65; padding: 10px 0 10px 26px; position: relative;
  border-bottom: 1px solid rgba(40,22,110,0.06);
  page-break-inside: avoid;
  break-inside: avoid;
}
...
.plus-minus-col h4 {
  ...
  color: var(--accent); margin-bottom: 6px; font-family: var(--font-body); font-weight: 600;
}
.pr-insight {
...
.pr-insight-label {
  ...
  font-weight: 700; color: var(--accent); margin-bottom: 6px; font-family: var(--font-body);
}
.pr-insight-text { font-size: 13.5px; line-height: 1.65; color: var(--ink); margin: 0; }

.closing-card {
  max-width: 600px; margin: 28px auto 0; padding: 32px 28px;
...
.mantra-line {
  ...
  color: var(--ink); line-height: 1.5; margin: 18px 0;
}
...
@media print {
  ...
  body::before {
    background: linear-gradient(180deg, #ECE7F8 0%, #DDD3F2 100%) !important;
  }
  .partner-reading {
    background: linear-gradient(180deg, #ECE7F8 0%, #DDD3F2 100%) !important;
    background-color: #ECE7F8 !important;
  ...
  /* Compact hero so section 1 can start on the same page */
  .hero {
  ...
  .hero-meta-row { padding: 8px 16px; font-size: 10px; }
  ...
  .pr-sub { margin-bottom: 14px !important; }
}
`;

export interface PartnerHtmlInput {
  content: any;
  fullName: string;
  dob?: string | null;
  /** Pre-formatted DD.MM.YYYY — preferred when already resolved by caller */
  dobDisplay?: string | null;
}

export function buildPartnerReadingHtml(input: PartnerHtmlInput): string {
  const { content, fullName, dob, dobDisplay: dobDisplayIn } = input;
  const firstName = (fullName || "Seeker").split(" ")[0] || "Seeker";
  const initial = (firstName[0] || "S").toUpperCase();
  const dobDisplay =
    (dobDisplayIn && dobDisplayIn !== "—" ? dobDisplayIn : null) ||
    formatDob(dob) ||
    formatDob(content?.metadata?.dob) ||
    "—";
  const age = calcAge(dob || content?.metadata?.dob || toIsoDob(dobDisplay));
  const sections: any[] = Array.isArray(content?.sections) ? content.sections : [];
  const chapterCount = Math.max(
    1,
    sections.filter((s) => s?.id && s.id !== "conclusion").length || sections.length,
  );
...
<style>
/* System fallbacks first so PDF renders even if Google Fonts are slow */
:root {
